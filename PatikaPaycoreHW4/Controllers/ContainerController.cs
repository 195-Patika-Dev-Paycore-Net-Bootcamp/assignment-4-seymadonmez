    ...
    public class ContainerController : ControllerBase
    {
        ...
        public IActionResult GetClusteredContainerListByKMeans(long vehicleId, int n)
        {
            ...
            while (--limit > 0)
            {
                ...
                {
                    var satir = resultSet[i]; //satır değişkeni mevcut veriyi tutmakta görevinde. Peşinden gelen oldCluster ise bu satır için atanan bir önceki kümeyi tutuyor.
                    var oldCluster = satir.AssignedCluster;
                    ...
                    var newCluster = centerPoints.Select(n => (clusterNumber: n.cluster,
                                                                    Distance: CalculateDistance(satir.Values, n.centerPoint)))
                    ...
                    {
                        resultSet[i] = (AssignedCluster: newCluster, Values: satir.Values);
                        isChanged = true;
                    }
                });
            ...
            return resultSet.Select(c=> (Cluster: c.AssignedCluster,  Values:c.Values.ToArray()));
        }
